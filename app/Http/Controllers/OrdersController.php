...
class OrdersController extends Controller
{
    ...
    public function cod(){
        ...
        Mail::send('payment.codmail', $messageData, function ($message) use($email){
            $message->to('$email')->subject('Order Placed - GarpStore');

        });
}
    public function banktransfer(Request $request){
        $who_buying=Orders_model::where('users_id',Auth::id())->first();
        return view('payment.banktransfer',compact('who_buying'));
}
